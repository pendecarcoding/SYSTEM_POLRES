                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <a data-toggle="modal" data-target="#tambah"
                            style="color:white;"class="btn waves-effect waves-light btn-primary">Tambah Data</a>
                        <!-- Modal -->
                                ...
                                <div class="modal-content">
                                    ...
                                    <form action="{{ url('settingroute/add') }}" method="post">{{ csrf_field() }}
                                        <div class="modal-body">
                                            ...
                                            <select class="form-control select2" name="akses[]" id="exampleSelect2"
                                                multiple="" style="width: 100%;color:white;">
                                            ...
                                            <td>
                                                <span data-toggle="tooltip" data-placement="top" title="Aksi Untuk Edit">
                                                    <a class="btn btn-warning btn-sm" data-toggle="modal"
                                                        data-target="#m{{ $v->id_route }}"><i style="color:white;"
                                                            class="fa fa-pencil"></i> </a>&nbsp;
                                                </span>
                                                <a data-toggle="tooltip" data-placement="top" title="Aksi Untuk Hapus"
                                                    onclick="return confirm('Hapus?')"
                                                    href="{{ url('settingroute/hapus/' . base64_encode($v->id_route)) }}"
                                        ...
                                        <div class="modal fade" id="m{{ $v->id_route }}" role="dialog">
                                            <div class="modal-dialog modal-lg">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ url('settingroute/update') }}" method="post">
                                                        {{ csrf_field() }}
                                                        <div class="modal-body">
                                                            <label>Type</label>
                                                            <input type="hidden" value="{{ $v->id_route }}"
                                                                class="form-control" name="id" required>
                                                            <input type="text" value="{{ $v->type }}"
                                                                class="form-control" name="type" required>
                                                            <label>Link</label>
                                                            <input type="text" value="{{ $v->link }}"
                                                                class="form-control" name="link" required>
                                                            <label>Controller</label>
                                                            <input type="text" value="{{ $v->controller }}"
                                                                class="form-control" name="controller" required>
                                                            <label>Method</label>
                                                            <input type="text" value="{{ $v->method }}"
                                                                class="form-control" name="method" required>
                                                            <label>Active</label>
                                                            <select class="form-control" name="active" required>
                                                                <option value="">--Pilih Aksi--</option>
                                                                <option value="Y"
                                                                    @if ($v->active == 'Y') selected @endif>Aktif
                                                                </option>
                                                                <option value="N"
                                                                    @if ($v->active == 'N') selected @endif>Non
                                                                    Aktif</option>
                                                            </select>
                                                            <label>Role Session</label>
                                                            <?php
                                                            ...
                                                            <select class="form-control select2" name="akses[]"
                                                                id="exampleSelect2" multiple=""
                                                                style="width: 100%;color:white;">
                                                                @foreach ($level as $index => $v)
                                                                    <option value="{{ $v->level }}"
                                                                        @if (in_array($v->level, $mn)) {{ 'Selected' }} @endif>
                                                                        {{ $v->level }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="modal-footer">
                                                            <a style="color:white;" class="btn btn-default"
